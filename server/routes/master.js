...
router.post('/customers', authenticate, authorize('admin', 'manager'), (req, res) => {
  const { customer_id, service_id, company_name, brand_site, address, service_type, grade, support_level, link_coverage, sla, latitude, longitude, city } = req.body;
  try {
    const r = db.prepare('INSERT INTO master_customer (customer_id, service_id, company_name, brand_site, address, service_type, grade, support_level, link_coverage, sla, latitude, longitude, city) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)').run(customer_id, service_id, company_name, brand_site, address || null, service_type || null, grade || null, support_level || null, link_coverage || null, sla || null, latitude || null, longitude || null, city || null);
...
router.post('/customers/batch', authenticate, authorize('admin', 'manager'), (req, res) => {
  ...
  const insert = db.prepare(`
    INSERT INTO master_customer (customer_id, service_id, company_name, brand_site, address, service_type, grade, support_level, link_coverage, sla, latitude, longitude, city) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  ...
  const insertMany = db.transaction((rows) => {
    ...
    for (const row of rows) {
      ...
      insert.run(
        ...
        row.latitude || null, row.longitude || null, row.city || null
      );
      count++;
    }
    return count;
...
router.put('/customers/:id', authenticate, authorize('admin', 'manager'), (req, res) => {
  const { customer_id, service_id, company_name, brand_site, address, service_type, grade, support_level, link_coverage, is_active, sla, latitude, longitude, city } = req.body;
  try {
    db.prepare('UPDATE master_customer SET customer_id = COALESCE(?, customer_id), service_id = COALESCE(?, service_id), company_name = COALESCE(?, company_name), brand_site = COALESCE(?, brand_site), address = COALESCE(?, address), service_type = COALESCE(?, service_type), grade = COALESCE(?, grade), support_level = COALESCE(?, support_level), link_coverage = COALESCE(?, link_coverage), sla = COALESCE(?, sla), latitude = COALESCE(?, latitude), longitude = COALESCE(?, longitude), city = COALESCE(?, city), is_active = COALESCE(?, is_active) WHERE id = ?').run(customer_id ?? null, service_id ?? null, company_name ?? null, brand_site ?? null, address ?? null, service_type ?? null, grade ?? null, support_level ?? null, link_coverage ?? null, sla ?? null, latitude ?? null, longitude ?? null, city ?? null, is_active ?? null, req.params.id);
