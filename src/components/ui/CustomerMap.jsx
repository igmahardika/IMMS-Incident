...
import GeoSummary from './GeoSummary.jsx';
import {
  CustomerGeocodeReportCard,
  CustomerPopup,
  CustomerTroubleSummaryCard,
...
export default function CustomerMap({
  ...
  return (
    <div className="map-surface flex h-full min-h-[640px] flex-1 flex-col overflow-hidden rounded-xl border border-border bg-muted/20">
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:flex xl:flex-wrap xl:justify-end">
              ...
              />
              <StatChip
                label={mapMode === 'trouble' ? 'Unmapped' : 'Missing Coords'}
                value={mapMode === 'trouble' ? Math.max(troubleData.length - troublePoints.length, 0) : missingCustomerCoords}
              />
              ...
              <Input
                ...
                placeholder={
                  mapMode === 'trouble'
                    ...
                    : 'Search address, city, province, OSC, ODC, ODP, or service ID'
                }
                className="pl-9"
              />
            </div>
      ...
      <div className="grid min-h-0 flex-1 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="relative min-h-[460px] xl:min-h-0">
          <MapContainer
          ...
          <SectionCard
            title={mapMode === 'trouble' ? 'Incident Density' : 'Coverage Legend'}
            className="pointer-events-none absolute bottom-4 left-4 z-[1000] min-w-[220px] max-w-[280px] border-border/90 bg-background/92 shadow-sm backdrop-blur"
          >
        ...
        <div className="flex min-h-0 flex-col gap-4 border-t border-border bg-card/70 p-4 xl:border-l xl:border-t-0">
          {mapMode === 'customers' ? (
            <>
              <CustomerGeocodeReportCard report={geocodeReport} />
              <GeoSummary customers={filteredCustomers} embedded className="min-h-0 flex-1" />
            </>
          ) : (
            <CustomerTroubleSummaryCard troublePoints={troublePoints} />
          )}
        </div>
      </div>
    </div>
